...
class BatteryTelemetry:
    ...
    def stop(self):
        """Safely stops the telemetry loops during application destruction."""
    ...
    def _telemetry_worker(self):
        ...
        while self.telemetry_alive:
            if HAS_BATTERY_DRIVERS:
                try:
                    battery = psutil.sensors_battery()
                    if battery is not None:
                        self.current_battery_pct = int(battery.percent)
                        # STRICT REQUIREMENT: Only flag True if under 20% AND unplugged
            ...
            # If things are healthy, update the normal layout components
            if not self.is_low_battery and self.telemetry_alive:
                self.app.update_status_text(self.get_status_string(), color="#666666")
                self.app.update_battery_display(f"BATTERY LIFE: {self.current_battery_pct}%")
    ...
    def _execute_ui_pulse_loop(self):
        ...
        if self.is_low_battery:
            ...
            # When low, the main status row converts into the critical warning ticker
            warning_text = "▪ VOLTAGE DROP CRITICAL ▪"
            self.app.update_status_text(warning_text, color=hex_color)
            self.app.update_battery_display(f"BATTERY LIFE: {self.current_battery_pct}%", color=hex_color)
        else:
            # Cleanly recover back to default gray if plug is re-introduced or battery is high
            if self.app.sub_text_id is not None:
                ...
                if current_color.startswith("#") and current_color != "#666666":
                    self.app.update_status_text(self.get_status_string(), color="#666666")
                    self.app.update_battery_display(f"BATTERY LIFE: {self.current_battery_pct}%", color="#666666")
